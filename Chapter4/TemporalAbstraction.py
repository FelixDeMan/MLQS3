...
class NumericalAbstraction:
    ...
    def smoothed_avg(self, data, window, window_size, p = 0.1):
        q = 1 - 2*p

        averages = data.rolling(window, min_periods=window_size).mean().tolist()

        smoothed_avgs = []
        for i, avg in enumerate(averages):
            if i == 0:
                smoothed_avg =  (1-p) * averages[i] + p * averages[i+1]
            if i == ( len(averages) - 1):
                smoothed_avg = (1 - p) * averages[i] + p * averages[i - 1]
            else:
                smoothed_avg = q * averages[i] + p * averages[i - 1] + p * averages[i + 1]

            smoothed_avgs.append(smoothed_avg)

        smoothed = pd.Series(smoothed_avgs, index = data.index)

        return smoothed

    def imputed_avg(self, data, window, window_size):
        ...
        for i, avg in enumerate(averages):
            if i == 0:
                imputed_avg =  averages[i+1]
            if i == ( len(averages) - 1):
                imputed_avg =  averages[i - 1]
            else:
                imputed_avg = 0.5 * averages[i - 1] + 0.5 * averages[i + 1]

            imputed_avgs.append(imputed_avg)

        #print(averages)
        #print(smoothed_avgs)
        imputed = pd.Series(imputed_avgs, index = data.index)
        print(imputed)
        return imputed
    ...
    def aggregate_value(self,data, window_size, aggregation_function):
        ...
        print(window_size, aggregation_function)
        ...
        if aggregation_function == 'mean':
            ...
            return data.rolling(2, min_periods=1).apply(np.std)
        elif aggregation_function == 'slope':
            return data.rolling(window, min_periods=1).apply(self.get_slope)
        

        else:
            if aggregation_function == 'smoothed':
                return self.smoothed_avg(data, window, window_size)
            if aggregation_function == 'imputed':
                return self.imputed_avg(data, window, window_size)
