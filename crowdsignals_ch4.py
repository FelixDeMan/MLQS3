...
DATA_PATH = Path('')
DATASET_FNAME = 'chapter3_result_final_own31.csv'
RESULT_FNAME = 'chapter4_result_own.csv'
...
def main():
    ...
    milliseconds_per_instance = (
        int(round((dataset.index[1] - dataset.index[0]).total_seconds() * 1e3)))
    ...
    if FLAGS.mode == 'aggregation':
        ...
        for ws in window_sizes:
            dataset = NumAbs.abstract_numerical(dataset, ['acc_phone_x'], ws, 'imputed')
            dataset = NumAbs.abstract_numerical(dataset, ['acc_phone_x'], ws, 'smoothed')
            #dataset = NumAbs.abstract_numerical(dataset, ['acc_phone_x'], ws, 'std')

        DataViz.plot_dataset(dataset, ['acc_phone_x', 'acc_phone_x_temp_imputed', 'acc_phone_x_temp_smoothed', 'label'], ['exact', 'like', 'like', 'like'], ['line', 'line', 'line', 'points'])
    ...
    if FLAGS.mode == 'frequency':
        ...
        dataset = FreqAbs.smoothed_avg(dataset, ['acc_phone_x'])
        # Spectral analysis.
        DataViz.plot_dataset(dataset, ['acc_phone_x_max_freq', 'engineered', 'acc_phone_x_pse', 'label'], ['like', 'like', 'like', 'like'], ['line', 'line', 'line','points'])
        print("--- %s seconds ---" % (time.time() - start_time))
