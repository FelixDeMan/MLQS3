...
DATA_PATH = Path('')
DATASET_FNAME = 'chosen_cols.csv'
RESULT_FNAME = 'chapter3_result_outliers_3.csv'
    ...
    if FLAGS.mode == 'chauvenet':
        for i in range(10):
            ...
            for col in outlier_columns:
                ...
                dataset = OutlierDistr.chauvenet(dataset, col, i+1)#FLAGS.C)
                DataViz.plot_binary_outliers(
                    dataset, col, col + '_outlier', k = (i+1), figname= 'Chauvenet C = ' + str(i+1) )
       ...
       dmins = np.linspace(FLAGS.dmin - 0.0, FLAGS.dmin + 0.4, 3)
       print(dmins)
       fmins = np.linspace(FLAGS.fmin - 0.4, FLAGS.fmin + 1, 3)
       print(fmins)
       for i in dmins:
        for j in fmins:
            print(i,j)
            ...
            #             dataset, [col], 'euclidean', i, j)#FLAGS.dmin, FLAGS.fmin)
            #         DataViz.plot_binary_outliers(
            #             dataset, col, 'simple_dist_outlier', figname='Simple dist outlier with dmin = '+ str(i) + ' and fmin = ' + (str(j)))
            ...
            for col in outlier_columns:
                try:
                    dataset = OutlierDist.local_outlier_factor(
                        dataset, [col], 'euclidean', k) #FLAGS.K)
                    DataViz.plot_dataset(dataset, [col, 'lof'], [
                                         'exact', 'exact'], ['line', 'points'], figname= 'LOF outliers of ' + str(col) + ' with k = ' + str(k))
                except MemoryError as e:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='chauvenet',
