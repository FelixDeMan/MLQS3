...
class PrepareDatasetForLearning:
    ...
    def assign_label1(self, dataset, class_labels):
        # Find which columns are relevant based on the possibly partial class_label
        # specification.
        labels = []
        for i in range(0, len(class_labels)):
        ...
        for i in range(0, len(dataset.index)):
            ...
            if sum_values[i] == 1:
                dataset.iloc[i, dataset.columns.get_loc(self.class_col)] = dataset[labels].iloc[i].idxmax(axis=1)
                #print(dataset.iloc[i, dataset.columns.get_loc(self.class_col)] )
            else:
                if sum_values[i] > 1:
                    dataset.iloc[i, dataset.columns.get_loc(self.class_col)] = 'multiple'
                if sum_values[i] == 0:
                    dataset.iloc[i, dataset.columns.get_loc(self.class_col)] = 'none'
                print(dataset.iloc[i, dataset.columns.get_loc(self.class_col)])
                # And remove our old binary columns.
        dataset = dataset.drop(labels, axis=1)
        return dataset
    ...
    def split_single_dataset_classification1(self, dataset, class_labels, matching, training_frac, filter=True,
                                            temporal=False, random_state=0):
        # Create a single class column if we have the 'like' option.
        if matching == 'like':
            dataset = self.assign_label1(dataset, class_labels)
            class_labels = self.class_col
        elif len(class_labels) == 1:
            class_labels = class_labels[0]
