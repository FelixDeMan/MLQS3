...
DATA_PATH = Path('')
DATASET_FNAME = 'chapter3_result_outliers_3.csv'
RESULT_FNAME = 'chapter3_result_final_own31.csv'
ORIG_DATASET_FNAME = 'chosen_cols.csv'
...
def main():
    ...
    if FLAGS.mode == 'imputation':
        ...
        imputed_model = MisVal.impute_model(copy.deepcopy(dataset), 'hr_watch_rate')
        #DataViz.plot_imputed_values(dataset, ['imputed_model'],'hr_watch_rate', imputed_model['hr_watch_rate'])
        DataViz.plot_imputed_values(dataset, ['original', 'mean', 'median', 'interpolation', 'imputed_model'], 'hr_watch_rate',
                                    imputed_mean_dataset['hr_watch_rate'],
                                    imputed_median_dataset['hr_watch_rate'],
                                    imputed_interpolation_dataset['hr_watch_rate'],
                                    imputed_model['hr_watch_rate'])
       ...
       # Determine the sampling frequency.
        ...
        n_pcs = 5 # 5 or 3, see fig None.png
