...
#                                                                   gridsearch=False)
#
# #plot shows we need 3 features
# DataViz.plot_xy(x=[range(1, N_FORWARD_SELECTION+1)], y=[ordered_scores],
#                 xlabel='number of features', ylabel='accuracy', fig_name='feature_importance')
#
#
# # based on python2 features, slightly different.
# selected_features = ordered_features[:3]




selected_features = ['mag_phone_y_freq_0.0_Hz_ws_2', 'mag_phone_z', 'acc_watch_x_freq_0.0_Hz_ws_2']
...
#                 names=['training', 'test'], line_styles=['r-', 'b:'], fig_name='reg_plot')
#
...
#best val = 10
leaf_settings = [1, 2, 5, 10, 11, 12, 13, 14, 15, 20]
performance_training = []
performance_test = []
...
DataViz.plot_xy(x=[leaf_settings, leaf_settings], y=[performance_training, performance_test],
                ...
                names=['training', 'test'], line_styles=['r-', 'b:'], fig_name='points_per_leaf')
...
for i in range(0, len(possible_feature_sets)):
    ...
    for repeat in range(0, N_KCV_REPEATS):
        ...
        class_train_y, class_test_y, class_train_prob_y, class_test_prob_y = learner.feedforward_neural_network(
            selected_train_X, train_y, selected_test_X, gridsearch=True, alpha= 0.0001
        )
...
#     #And we run our deterministic classifiers:
    ...
    class_train_y, class_test_y, class_train_prob_y, class_test_prob_y = learner.decision_tree(
        selected_train_X, train_y, selected_test_X, gridsearch=True, min_samples_leaf= 10
    )
...
DataViz.plot_performances_classification(['NN', 'RF','SVM', 'KNN', 'DT', 'NB'], feature_names, scores_over_all_algs, fig_name='perf_models')
...
DataViz.plot_confusion_matrix(test_cm, class_train_prob_y.columns, normalize=False, fig_name='confusion_matrix')

#run on crowdsignal data